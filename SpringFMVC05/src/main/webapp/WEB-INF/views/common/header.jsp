  	  ...
  	  <c:if test="${!empty mvo}">
	      <ul class="nav navbar-nav navbar-right">
            ...
            <c:if test="${!empty mvo }"> <!-- 로그인을 한 경우 -->
			  	<c:if test="${empty mvo.memProfile}"> <!-- Profile이 null인 경우 -->
			  		<li><img class="img-circle" alt="기본이미지" src="${contextPath}/resources/images/person.png" style="width: 50px; height: 50px"/>${mvo.memName}님 
			  		(<!-- 회원로그인시 해당 회원의 권한 표시 -->
			  	...
			  	<c:if test="${!empty mvo.memProfile}"> <!-- Profile이 null이 아닌 경우 -->
			  		<li><img class="img-circle" alt="회원이미지" src="${contextPath}/resources/upload/${mvo.memProfile}" style="width: 50px; height: 50px"/>${mvo.memName}님 
			  		(
			  		<c:forEach var="authVO" items="${mvo.authList}">
			  			<c:if test="${authVO.auth eq 'ROLE_USER'}">U</c:if>
			  			<c:if test="${authVO.auth eq 'ROLE_MANAGER'}">M</c:if>
